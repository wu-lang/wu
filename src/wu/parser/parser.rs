        ...
        match self.current_lexeme().as_str() {
          ":" => {
            ...
            if let Some(right) = self.parse_right_hand(name.clone())? {
              Statement::new(
                StatementNode::Variable(Type::from(TypeNode::Nil), name, Some(right)),
              ...
              if self.current_lexeme() == "=" {
                ...
                Statement::new(
                  StatementNode::Variable(kind, name, Some(self.parse_expression()?)),
                  self.span_from(position),
                )
              } else {
                Statement::new(
  ...
  fn parse_right_hand(&mut self, name: String) -> Result<Option<Expression>, ()> {
    let declaration = match self.current_lexeme().as_str() {
      ...
      "struct" => {
        ...
        Some(Expression::new(
          ExpressionNode::Struct(name, params, format!("{}{}", self.source.file, position)),
          position,
        ))
      }
          ...
          match self.current_lexeme().as_str() {
            ...
            "switch" => {
              ...
              let decl = Statement::new(
                StatementNode::Variable(Type::from(TypeNode::Nil), name.clone(), Some(right_hand)),
                position.clone(),
              );
            ...
            "while" => {
              ...
              Expression::new(
                ExpressionNode::While(Rc::new(condition), Rc::new(self.parse_expression()?)),
                position,
              )
            }

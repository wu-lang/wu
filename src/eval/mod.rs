        ...
        match &(*called).node {
          Identifier(id) => {
            ...
            let arg_vals = args
              ...
              .map(|arg| {
                self
                  .eval_expression(ctx, Rc::new(arg.clone()))
                  .unwrap_or_else(|| panic!("can't pass as function arg: {}", arg.pos))
              })
              .collect::<Vec<_>>();
            ...
            if self.builtins.contains_key(id) {
              ...
              let fun = self
                .fetch(ctx, &id)
                .unwrap_or_else(|| panic!("No function with the name \"{}\" found", id));

              // get the arguments and block out of the function
              let (args, _, block, _) = fun.function_ref().expect("Can only call functions");
